...
class AttendanceController {
    ...
    async getAllStudentsWithAttendance(req, res) {
        try {
            const studentsWithAttendance = await this.attendanceRepository.getAllStudentsWithAttendance();
    ...
    async getUserAttendancesWithClassPlans(req, res) {
        ...
        try {
            const attendances = await this.attendanceRepository.getUserAttendancesWithClassPlans(studentId);
            return res.status(200).json(attendances);
        }
        catch (error) {
...
const createAttendanceController = () => {
    const attendanceRepository = new AttendanceRepository_1.AttendanceRepository(); // Cria a instância do repositório
